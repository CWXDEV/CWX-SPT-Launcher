...
public class GameHelper
{
    ...
    public async Task<bool> LaunchGame()
    {
        ...
        var clientProcess = new ProcessStartInfo(clientExecutable)
        {
            ...
            WorkingDirectory = _serverHelper.ConnectedServer.GamePath,
        };
    ...
    bool IsInstalledInLive()
    {
        ...
        try
        {
            ...
            [
                // SPT files
                new FileInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, "SPT.Launcher.exe")),
                new FileInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, "SPT.Server.exe")),

                // bepinex files
                new FileInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, @"doorstep_config.ini")),
                new FileInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, @"winhttp.dll")),

                // licenses
                new FileInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, @"LICENSE-BEPINEX.txt")),
                new FileInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, @"LICENSE-ConfigurationManager.txt")),
                new FileInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, @"LICENSE-Launcher.txt")),
                new FileInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, @"LICENSE-Modules.txt")),
                new FileInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, @"LICENSE-Server.txt"))
            ];
            DirectoryInfo[] directories =
            [
                new DirectoryInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, @"SPT_Data")),
                new DirectoryInfo(Path.Combine(_serverHelper.ConnectedServer.GamePath, @"BepInEx"))
            ];
    ...
    private string GetFileForCleanup(string fileName)
    {
        ...
        return Path.Join(_serverHelper.ConnectedServer.GamePath, fileName);
    }
    ...
    {
        var rootdir = new DirectoryInfo(Path.Join(_serverHelper.ConnectedServer.GamePath, "user\\sptappdata"));
